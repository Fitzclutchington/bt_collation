...
clear_samples_compare(const Mat1f &bt11, const Mat1f &bt11_clear, const Mat1b &l2p_mask,
                      Mat1b &bt11_masked,int cur_mid, string filename)
{
    int x,y,t,time;
    int t_lag0 = 6;
    int t_lag1 = 2;
    ...
    for(y = 0; y < HEIGHT; ++y){
        for(x = 0; x < WIDTH; ++x){
            if(std::isfinite(TL1(y, x)) && std::isfinite(TL2(y, x))){
                if( bt11(y, x, cur_mid) > TL0(y, x) - PASS2 && ((TL2(y, x)-TL1(y, x) < PASS2 && std::isfinite(bt11_clear(y, x, cur_mid))))){ 
                    bt11_masked(y, x) = 255;
                }
    ...
    string TL0_filename = "data/TL0" + filename;
    string TL1_filename = "data/TL1" + filename;
    string TL2_filename = "data/TL2" + filename;

    save_test_nc_float(TL0, TL0_filename.c_str() );
    save_test_nc_float(TL1, TL1_filename.c_str());
    save_test_nc_float(TL2, TL2_filename.c_str());
...
second_pass(const vector<string> &clear_paths, const vector<string> &original_paths, vector<string> &second_pass_paths, Mat1b &l2p_mask)
{
    ...
    clear_samples_compare(sst_samples, clear_samples, l2p_mask, clear2, cur_mid,filename); 	
    file_count++;
